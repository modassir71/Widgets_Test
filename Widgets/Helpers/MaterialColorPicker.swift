...
open class MaterialColorPicker: UIView, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
  ...
  @objc dynamic open var dataSource: MaterialColorPickerDataSource?{
    didSet{
  ...
  @objc dynamic open var delegate: MaterialColorPickerDelegate?
...
//  }
  ...
  //Select index programtically
    open func selectColor(_ color: String){
        let index = colors.lastIndex(where: {$0.hexStringFromColor() == color})
        selectedIndex = IndexPath(item: index ?? 0, section: 0)
